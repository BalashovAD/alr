...
/***/ function(module, exports, __webpack_require__) {
	...
	function filter(obj, checker) {
		var tmp = Object.create(obj.__proto__);
	...
	function Helper(input, container, emptySingleContainer) {
	  ...
	  * }
	  */
		var FUNC = JSON.parse($('#mainFunc').html());
		/**
	  * PARAMS = [
	  *     $PARAM_NAME%: {
	  ...
	  */
		...
		function check(val) {
			var funcName = val.split(' ')[0];
	
			return typeof FUNC[funcName] != 'undefined';
		}
	
		function show(objWithDescription) {
			var title = objWithDescription.title || '';
	
			emptySingleContainer(container);
	
			container.append($('<span>').html(title + ' = ').addClass('object-name')).append($('<span>').addClass('open-curly-brace'));
	
			var data = $('<ol>').addClass('single-data');
	
			if (typeof objWithDescription.func != 'undefined') {
				// show all func with description
				var li = void 0;
	
				for (var i in objWithDescription.func) {
					if (objWithDescription.func.hasOwnProperty(i)) {
						li = $('<li>').append($('<span>').html(i).addClass('object-name')).append($('<span>').html(' [setup]').data('func', i).click(setupFunc)).append($('<span>').html(' : ')).append($('<span>').html(objWithDescription.func[i].description));
					}
					data.append(li);
				}
			} else {
				...
				_li = $('<li>').append($('<span>').html('description').addClass('object-name')).append($('<span>').html(' : ')).append($('<span>').html(objWithDescription.description));
	
				data.append(_li);
	
				for (var _i in objWithDescription.params) {
					if (objWithDescription.params.hasOwnProperty(_i)) {
						_li = $('<li>').append($('<span>').html(_i).addClass('object-name')).append($('<span>').html(' : ')).append($('<span>').html(objWithDescription.params[_i]));
					}
					data.append(_li);
				}
			}
			...
			if (!isHelpMode) {
				...
				input.on('input.help', function () {
					...
					if (params.length > 0 || typeof FUNC[cmd] !== 'undefined') {
						...
						(function () {
							// choose cmd
							var regExp_EqualStartOfFuncName = new RegExp('^' + cmd, 'i');
	
							var allowedFunc = filter(FUNC, function (name) {
				...
				runOnKeys(function () {
					...
					if (params.length == 0) {
						(function () {
							...
							var allowedFunc = Object.keys(FUNC).filter(function (name) {
								return regExp_EqualStartOfFuncName.test(name);
							});
	...
	function Adm() {
	  ...
	  */
		function parseObjectToHTML(data, lvl) {
			...
			var txt = $('<ol>').addClass('single-data');
			var li = void 0;
					...
					switch (_typeof(data[i])) {
						...
						case 'object':
							if (typeof data[i][0] === 'undefined') {
								// object
								li = $('<li>').append($('<span>').html(i + ' = ').addClass('object-name')).append($('<span>').addClass('open-curly-brace')).append(parseObjectToHTML(data[i], lvl + 1));
								txt.append(li);
	
								txt.append($('<span>').addClass('close-curly-brace'));
							} else {
								// array
								var showAllBtn = $('<span>').html('[show]').click(showAllElemsOfArray).addClass();
								var countOfElems = $('<span>').html('(' + data[i].length + 'elems)');
	
								li = $('<li>').append($('<span>').html(i + ' = ').addClass('object-name')).append(showAllBtn).append(countOfElems).append($('<span>').addClass('open-square-bracket')).append(parseArrayToHTML(data[i], lvl + 1));
								txt.append(li);
	  ...
	  */
		function parseArrayToHTML(data, lvl) {
			...
			var txt = $('<ol>').addClass('single-data').addClass('array-data');
			var li = void 0;
				...
				if (data.hasOwnProperty(i)) {
					...
					switch (_typeof(data[i])) {
						...
						case 'object':
							if (typeof data[i][0] === 'undefined') {
								// object
								li = $('<li>').addClass(i + 1 > MAX_DISPLAY_ARRAY_ELEMS ? 'hidden-array-elem' : '').append($('<span>').addClass('open-curly-brace')).append(parseObjectToHTML(data[i], lvl + 1)).append($('<span>').addClass('close-curly-brace').addClass('elems-of-array' + (isLastElem ? ' last-elems-of-array' : '')));
								txt.append(li);
							} else {
								// array
								var showAllBtn = $('<span>').html('[show]').click(showAllElemsOfArray).addClass();
	
								li = $('<li>').addClass(i + 1 > MAX_DISPLAY_ARRAY_ELEMS ? 'hidden' : '').append(showAllBtn).append($('<span>').addClass('open-square-bracket')).append(parseArrayToHTML(data[i], lvl + 1)).append($('<span>').addClass('close-square-bracket').addClass('elems-of-array' + (isLastElem ? ' last-elems-of-array' : '')));
								txt.append(li);
							}
	   ...
	   */
			...
			var txt = $('<ol>').addClass('main-container');
	
			for (var i = 0; i < d.length; ++i) {
				var li = $('<li>').addClass('object');
	
				var idEl = $('<span>').html('[id]').data('id', d[i]._id).data('col', sel).click(getById);
				li.append(idEl);
	
				var delEl = $('<span>').html('[del]').data('id', d[i]._id).data('col', sel).click(deleteById);
				li.append(delEl);
	
				for (var k in mainFiends[sel]) {
					if (mainFiends[sel].hasOwnProperty(k)) {
						if (d[i] && d[i][mainFiends[sel][k]] && typeof d[i][mainFiends[sel][k]] === 'string' || typeof d[i][mainFiends[sel][k]] === 'number') {
							var span = $('<span class="object-name">' + mainFiends[sel][k] + '</span> : <span class="object-value">' + d[i][mainFiends[sel][k]] + '</span> <span class="invisibility">!</span>');
	
							li.append(span);
						} else if (d[i] && d[i][mainFiends[sel][k]] && _typeof(d[i][mainFiends[sel][k]]) === 'object') {
							for (var kk in d[i][mainFiends[sel][k]]) {
								if (d[i][mainFiends[sel][k]].hasOwnProperty(kk)) {
									var _span = $('<span class="object-name">' + mainFiends[sel][k] + '.' + kk + '</span> : <span class="object-value">' + d[i][mainFiends[sel][k]][kk] + '</span> <span class="invisibility">!</span>');
		...
		function parseSchema(data) {
			...
			var pref = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
					...
					switch (_typeof(data[i])) {
						...
						case 'object':
							if (data.hasOwnProperty('__' + i) && typeof data['__' + i] != 'undefined' && data['__' + i] === 'array') {} else {
								li = $('<li>').append($('<span>').html(i + ' = ').addClass('object-name')).append($('<span>').addClass('open-curly-brace')).append(parseSchema(data[i], lvl + 1, pref + i + '.'));
								txt.append(li);
		...
		function showMessage(data) {
			...
			el.append($('<span>').html(data.cmdLine + ' = ').addClass('object-name')).append($('<span>').addClass('open-curly-brace'));
	
			data.cmdLine = undefined;
	
			el.append(parseObjectToHTML(data)).attr('class', 'single').append($('<span>').addClass('close-curly-brace'));
		}
		...
		function addDataInSingleContainer(data, el, nowSelect) {
			...
			el.append($('<span>').html(nowSelect + ' = ').addClass('object-name')).append(delEl).append($('<span>').addClass('open-curly-brace'));
			el.append(parseObjectToHTML(data)).attr('class', 'single-' + nowSelect + ' single').append($('<span>').addClass('close-curly-brace'));
		}
		...
		this.add = function (side, col) {
			...
			$.ajax({
				...
				singleContainer[side].append($('<span>').html(col + ' = ').addClass('object-name')).append(addEl).append(resetEl).append($('<span>').addClass('open-curly-brace'));
				singleContainer[side].append(parseSchema(d, col)).attr('class', 'single-' + col + ' single').append($('<span>').addClass('close-curly-brace'));
